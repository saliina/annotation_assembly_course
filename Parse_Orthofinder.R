...
dat <- fread("/data/users/sjaegers/assembly_annotation_course/12_Genespace_all/orthofinder/Results_Nov19/Comparative_Genomics_Statistics/Statistics_PerSpecies.tsv")
ids=names(dat)
...
p <- ggplot(o_ratio, aes(x =  V1, y = perc, fill = species)) +
  ...
  labs(y = "Count")
ggsave("plots/orthogroup_number_plot.pdf")
...
p <- ggplot(o_percent, aes(x =  V1, y = perc, fill = species)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(y = "Count")
ggsave("plots/orthogroup_percentage_plot.pdf")
...
ogroups <- fread("/data/users/sjaegers/assembly_annotation_course/12_Genespace_all/orthofinder/Results_Nov19/Orthogroups/Orthogroups.GeneCount.tsv")
...
#genomes <- c('An_1','C24','Cvi_0','Ler_3','Sha','TAIR10')
genomes <- ids[-1]
...
pdf("one-to-one_orthogroups_plot.complexupset.pdf", height = 7, width = 20, useDingbats = FALSE)
